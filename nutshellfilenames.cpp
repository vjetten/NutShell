...
bool nutshellqt::getScriptReport()
{
   if (tabWidget->currentIndex() >= 0)
   {
      ...
      // add the variables in the binding that are not reprted but
      ...
      for (int i = 0; i < binding.count(); i++)
      {
         ...
         if (!strtmp.contains(bind[0]))
         {
            ...
            // available in binding only, may or may not exist. Some are input, some are non reported output
            //reportRec.lineNumber = bind[2].toInt();
